...
class SongService {
    ...
    static func saveSong(song: SongModel, completion: @escaping (Result<SongModel, Error>) -> Void) {
        ...
        database.save(record) { (savedRecord, error) in
            ...
            let savedSong = SongModel(id: savedRecord["id"] ?? "", title: savedRecord["title"] ?? "", artist: savedRecord["artist"] ?? "", album: savedRecord["album"] ?? "", coverArt: savedRecord["coverArt"] ?? "")
